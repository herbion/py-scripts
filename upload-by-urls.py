...
default_path = r'D:\vk_api\empartridge.tumblr.com.txt'
...
pics = filter(lambda pic: any([pic.endswith(ext) for ext in ['jpg', 'jpeg', 'png']]), map(lambda p: p.strip(), open(pictures_path).readlines()))

raw_input("Found %d pics. Go? " % len(pics))

while len(pics) > 0:
	part = map(lambda x: x, pics[:5])
